#!/usr/bin/env python3
"""
Test 4: Force Draw True - Force every frame update
"""
...
pygame.display.set_caption("Test 4: Force Draw True")
...
print("=== Test 4: Force Draw True ===")
print("Force every frame update - watch for flickering")
...
try:
    ...
    while running and (time.time() - start_time) < 8 and video.active:
        ...
        screen.fill(BLACK)
        video.draw(screen, video_pos, force_draw=True)  # FORCE EVERY FRAME
        
        elapsed = time.time() - start_time
        remaining = max(0, 8 - int(elapsed))
        text = font.render(f"Force - {remaining}s", True, WHITE)
